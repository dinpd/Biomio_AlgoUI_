...
from defs import TRAINING_FULL
...
class IdentificationSAInterface(AlgorithmInterface):
    ...
    def training(self, **kwargs):
        ...
        if mode == TRAINING_FULL:
            train_process = TrainingProcess(TEMP_DATA_PATH, None)
            data_detect_process = DataDetectionProcess(TEMP_DATA_PATH)
            rotation_detect_process = RotationDetectionProcess(TEMP_DATA_PATH)
            rotation_result_process = RotationResultProcess(TEMP_DATA_PATH, None)
            cluster_matching_process = ClusterMatchingProcess(None)
