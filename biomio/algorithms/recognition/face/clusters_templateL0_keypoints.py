...
from biomio.algorithms.recognition.estimation import ClusterL0Estimation, SelfGraphEstimation, \
    ...
    SelfGraphDistanceEstimation, SelfGraphDatabaseEstimation, AverageDescriptorEstimation, \
    ClusterSelfGraphEstimation
...
def get_estimation():
    return ClusterL0Estimation
    # return SelfGraphEstimation
    # return SelfGraphDatabaseEstimation
    # return AverageDescriptorEstimation
    # return ClusterSelfGraphEstimation
...
class ClustersTemplateL0MatchingDetector(ClustersMatchingDetector):
    def __init__(self):
        ...
        self._etalon = get_estimation().defaultDatabase()
    ...
    def update_hash_templateL0(self, data):
        estimation = ClusterL0Estimation(self.kodsettings.detector_type, knn=3, max_distance=0.9,
                                         mode=CROSS_DISTANCE_MODE)
        # estimation = get_estimation()(self.kodsettings.detector_type, knn=3)
    ...
    def importSources(self, source):
        ...
        self._etalon = get_estimation().importDatabase(source.get('data', dict()))
    ...
    def load_database(source):
        return {
            "template": get_estimation().importDatabase(source.get('data', dict())),
            "threshold": source.get('threshold', 100)
        }

    def exportSources(self):
        data = get_estimation().exportDatabase(self._etalon)
        if len(data.keys()) > 0:
            return {
                'data': data,
    ...
    def verify_template_L0(self, data):
        estimation = ClusterL0Estimation(self.kodsettings.detector_type, knn=2, max_distance=0.9,
                                         mode=CROSS_DISTANCE_MODE)
        # estimation = get_estimation()(self.kodsettings.detector_type, knn=2)
